def summary_scrape(season,gameId,subSeason='02',*raw_html):
    ...
    if ~('raw_html' in locals()):
        ...
        try:
            ...
            return pd.DataFrame(['Game not found.'],columns=['Err']),pd.DataFrame([])
    bsObj=BeautifulSoup(raw_html.read().decode('utf-8'),'html.parser')
...
def season_summary_scrape(season,start=1,subSeason='02',autosave=False):
    ...
    while True:
        gameId = '%04d' % i
        try:
            raw_html=urlopen('http://www.nhl.com/scores/htmlreports/'+str(season)+'/GS'+str(subSeason).zfill(2)+gameId+'.HTM')
        except:
            ...
            print('Unable to find: ' + str(season[:4]) + str(subSeason) + str(gameId))
            if (i==failed+1):
                break
            else:
                failed=i
                i+=1
                continue
        print('Scraping game ' + str(season[:4]) + str(subSeason) + str(gameId))
        df_tmp,df_meta=summary_scrape(str(season),gameId,subSeason,raw_html)
    ...
    if autosave:
        ...
        season_gs_meta_df.to_csv('ss_'+str(season)+'_meta.csv',index=True)
...
def ss_df_import(season=2008):
    ...
    for szn in range(season,2019):
        df=df.append(pd.read_csv('csv\ss_'+str(szn)+str(szn+1)+'.csv',dtype={'gameId':'str'}).set_index(['Season','gameId']),sort=False)
